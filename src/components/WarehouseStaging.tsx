  ...
  return (
    <group position={position}>
      ...
      {Array.from({ length: bayCount + 1 }, (_, i) => {
        ...
        return (
          <group key={i}>
            <mesh position={[x, rackHeight / 2, 0.55]} castShadow dispose={null} material={matStructureSteel}>
      ...
      {Array.from({ length: levels }, (_, lvl) => {
        ...
        return (
          <group key={lvl}>
            <mesh position={[0, y, 0.55]} castShadow dispose={null} material={matRailYellow}>
...
function StretchWrapperStub({ position }: { position: V3 }) {
  return (
    <group position={position}>
      <mesh position={[0, 1.1, 0]} castShadow receiveShadow dispose={null} material={matPaintedSteel}>
        <boxGeometry args={[1.6, 2.2, 1.6]} />
      </mesh>
      {/* Turntable */}
      <mesh position={[0, 0.12, 0]} castShadow dispose={null} material={matStructureSteel}>
        <cylinderGeometry args={[0.75, 0.75, 0.12, 20]} />
      </mesh>
      {/* Film roll mast */}
      <mesh position={[0.7, 1.4, 0]} castShadow dispose={null} material={matStructureSteel}>
...
function TruckDock({ position }: { position: V3 }) {
  return (
    <group position={position}>
      {/* Dock pad */}
      <mesh position={[0, 0.05, 0]} receiveShadow dispose={null} material={matPaintDark}>
        <boxGeometry args={[4.5, 0.1, 3.5]} />
      </mesh>
      {/* Leveler plate */}
      <mesh position={[-1.8, 0.55, 0]} castShadow dispose={null} material={matStructureSteel}>
      ...
      <group position={[1.2, 0, 0]}>
        ...
        {[[-1.2, 0.35, 1.0], [-1.2, 0.35, -1.0], [1.0, 0.35, 1.0], [1.0, 0.35, -1.0]].map(
          (pos, i) => (
            <mesh key={i} position={pos as V3} rotation={[0, 0, Math.PI / 2]} dispose={null} material={matRubber}>
...
function AnimatedForklift({ active = true }: { active?: boolean }) {
  ...
  return (
    <group ref={groupRef}>
      <mesh position={[0, 0.55, 0]} castShadow dispose={null} material={matPaintYellow}>
        <boxGeometry args={[1.4, 1.0, 0.9]} />
      </mesh>
      <mesh position={[0.55, 0.85, 0]} castShadow dispose={null} material={matPaintDark}>
        <boxGeometry args={[0.5, 0.55, 0.85]} />
      </mesh>
      <mesh position={[-0.55, 1.2, 0]} castShadow dispose={null} material={matStructureSteel}>
        <boxGeometry args={[0.12, 2.0, 0.55]} />
      </mesh>
      <group name="forks" position={[0, 0.35, 0]}>
        <mesh position={[-1.15, 0, 0.22]} castShadow dispose={null} material={matSteel}>
