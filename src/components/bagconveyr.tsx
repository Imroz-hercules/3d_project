...
function ConveyorFrame({ length, width, height }: { length: number; width: number; height: number }) {
  ...
  return (
    <group>
      ...
      <mesh position={[0, height - 0.15, width / 2 - 0.08]} castShadow dispose={null} material={matPaintedSteel}>
      ...
      {Array.from({ length: 6 }, (_, i) => {
        ...
        return (
          <mesh key={i} position={[x, height - 0.15, 0]} castShadow dispose={null} material={matPaintedSteel}>
      ...
      {/* Adjustable Feet */}
      {legPositions.map((pos, i) => (
        <mesh key={`foot-${i}`} position={[pos[0], 0.05, pos[2]]} dispose={null} material={matStructureSteel}>
...
function BeltAndRollers({ length, width, height, active }: { length: number; width: number; height: number; active: boolean }) {
  ...
  return (
    <group>
      {/* Belt Surface */}
      <mesh ref={beltRef} position={[0, height - 0.05, 0]} castShadow receiveShadow dispose={null} material={matRubber}>
        <boxGeometry args={[length - 0.3, 0.05, width - 0.2]} />
      </mesh>

      {/* Drive Roller (Right end) */}
      <mesh ref={driveRollerRef} position={[length / 2 - 0.15, height - 0.1, 0]} rotation={[0, 0, Math.PI / 2]} castShadow dispose={null} material={matSteel}>
        <cylinderGeometry args={[0.12, 0.12, width - 0.25, 24]} />
      </mesh>

      {/* Tail Roller (Left end) */}
      <mesh ref={tailRollerRef} position={[-length / 2 + 0.15, height - 0.1, 0]} rotation={[0, 0, Math.PI / 2]} castShadow dispose={null} material={matSteel}>
        <cylinderGeometry args={[0.12, 0.12, width - 0.25, 24]} />
      </mesh>
      ...
      {[length / 2 - 0.15, -length / 2 + 0.15].map((x, i) => (
        <group key={i}>
          <mesh position={[x, height - 0.1, width / 2 - 0.15]} rotation={[0, 0, Math.PI / 2]} dispose={null} material={matSteelDark}>
...
function GearMotor({ position, active }: { position: V3; active: boolean }) {
  ...
  return (
    <group position={position}>
      ...
      {/* Fan Cover */}
      <mesh position={[0, 0, 0.22]} rotation={[0, 0, Math.PI / 2]} dispose={null} material={matPaintDark}>
        <cylinderGeometry args={[0.16, 0.16, 0.06, 24]} />
      </mesh>

      {/* Fan Blades */}
      <mesh ref={fanRef} position={[0, 0, 0.25]} rotation={[0, 0, Math.PI / 2]} dispose={null} material={matStructureSteel}>
        <cylinderGeometry args={[0.13, 0.13, 0.03, 8]} />
      </mesh>

      {/* Gearbox */}
      <mesh position={[0, 0, -0.25]} castShadow dispose={null} material={matPaintDark}>
        <boxGeometry args={[0.25, 0.25, 0.2]} />
      </mesh>

      {/* Shaft to Roller */}
      <mesh position={[0, 0, -0.35]} rotation={[0, 0, Math.PI / 2]} dispose={null} material={matSteel}>
...
function SideGuidesAndGuards({ length, width, height }: { length: number; width: number; height: number }) {
  return (
    <group>
      {/* Side Guide Rails */}
      <mesh position={[0, height + 0.05, width / 2 - 0.05]} castShadow dispose={null} material={matSteel}>
      ...
      <mesh position={[length / 2 - 0.15, height - 0.1, width / 2 + 0.1]} castShadow dispose={null} material={matRailYellow}>
...
function PhotoelectricSensor({ position, bagDetected }: { position: V3; bagDetected: boolean }) {
  return (
    <group position={position}>
      ...
      {/* Mounting Bracket */}
      <mesh position={[0, -0.08, 0]} dispose={null} material={matPaintedSteel}>
...
function CableTray({ length, width, height }: { length: number; width: number; height: number }) {
  return (
    <group position={[0, height * 0.5, -width / 2 - 0.15]}>
      ...
      {/* Tray Sides */}
      <mesh position={[0, 0.04, 0.05]} dispose={null} material={matPaintedSteel}>
